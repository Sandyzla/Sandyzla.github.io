<template>
  <TransitionGroup name="appear">
    <div v-for="cell in cells" :key="cell.id" class="cell" ref="cellsRef"
      :style="// calc(50% - 210px)
  ...
  <audio v-for="(mergeSrc, index) in mergeSrcs" :key="index" :src="mergeSrc" hidden preload="auto"
    ref="mergeMus"></audio>
  <audio v-for="(moveSrc, index) in moveSrcs" :key="index" :src="moveSrc" hidden preload="auto" ref="moveMus"></audio>
</template>

<script>
import move1_mp3 from '../../assets/2048/move1.mp3'
import move2_mp3 from '../../assets/2048/move2.mp3'
import conbine1_mp3 from '../../assets/2048/conbine1.mp3'
import conbine2_mp3 from '../../assets/2048/conbine2.mp3'

export default {
  emits: ['addScore', 'canUndo', 'gameOver'],
  ...
  methods: {
    ...
    //完成合并相关事项：
    doMergeStuff(disCell, toCell) {
      //disCell = disappear(合并后消失的元素), toCell(合并成的元素)
      //1.记录加分/待撤销分数
      ...
      this.delayStorey.unshift({
        ...
        cell_a: disCell,
        cell_b: toCell,
        dom: this.$refs.cellsRef[this.cells.indexOf(toCell)],
      })
    },
    //----------四个判断能否移动 结束----------end
    //----------四个方向移动 开始----------start
    moveUp() {
      for (let i = 0; i < 4; i++) {
        ...
        if (j == tmp.length - 1 && tmp[j].y != tmp[j - 1].y + 1) tmp[j].y = tmp[j - 1].y + 1
      }
    },
    moveDown() {
      for (let i = 0; i < 4; i++) {
    ...
    moveLeft() {
      for (let i = 0; i < 4; i++) {
        ...
        if (j == tmp.length - 1 && tmp[j].x != tmp[j - 1].x + 1) tmp[j].x = tmp[j - 1].x + 1
      }
    },
    moveRight() {
      for (let i = 0; i < 4; i++) {
    ...
    undoGame() {
      ...
      if (this.undoScore != 0) this.$emit('addScore', -this.undoScore)
    },
    afterMove() {
      //处理移动后的逻辑：是否加分、gameover判定
      this.$emit('canUndo', true)
      if (this.addScore != 0) {
        this.$emit('addScore', this.addScore) //将加分消息传递给父组件
        this.addScore = 0
      }
      // custom模式下的gameOver判定
  ...
  mounted() {
    ...
    document.addEventListener('keydown', (event) => {
      if (this.delayHandle() || this.gameOver) {
        //处理残余步骤
        return
      }
      switch (event.key.toLocaleUpperCase()) {
        ...
        case 'W':
          if (this.CanMoveUp()) {
            this.undoCells = JSON.parse(JSON.stringify(this.cells));
            this.undoScore = 0
            this.playMoveMusic()
            this.moveUp()
            this.afterMove()
  ...
  data() {
    return {
      ...
      moveSrcs: [move1_mp3, move2_mp3],
      mergeSrcs: [conbine1_mp3, conbine2_mp3],
      //延迟
...
.cell {
  ...
  letter-spacing: 1px;
  transition:
    0.16s top ease-in-out,
    0.16s left ease-in-out,
    0.3s scale;
}
