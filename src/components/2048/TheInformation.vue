  ...
  <div>
    <div class="show_grid">
      <div class="score">
        ...
        <label class="inNum"> {{ showScore.toFixed(0) }}</label>
      </div>
      <div class="best">
        <label class="inFont">BEST</label>
        <label class="inNum"> {{ best.toFixed(0) }}</label>
      </div>
      <div class="delete">
        <label class="inFont">DELETE</label>
        <label class="inNum"> {{ deleteTime }}</label>
      </div>
    </div>
    <div class="control_grid">
      <game-mode @set-mode="setGameMode"></game-mode>
      <label @click="clickUndo" :class="{ gameButton: canUndo, lockUndo: !canUndo }"
        style="grid-area: undo">UNDO</label>
    </div>
...
import record_mp3 from '../../assets/2048/record.mp3'
export default {
...
.gameButton:before {
  ...
  background: linear-gradient(90deg,
      rgba(250, 156, 112, 0.7737219887955182) 0%,
      rgba(99, 176, 250, 0.8661589635854342) 91%);
...
.quickShow-leave-active {
  transition: all 0.7s cubic-bezier(0.82, 0.13, 0.77, 0.62);
