<template>
  ...
  <TransitionGroup name="appearCard">
    <div v-for="cell in cells" :key="cell.id" class="card" ref="cellsRef"
      ...
      <div class="num"
        ...
        <div class="only_num">{{ cell.num }}</div>
    ...
    <svg width="164" height="109" xmlns="http://www.w3.org/2000/svg">
      <g>
        ...
        <TransitionGroup name="dirTip">
          <rect v-if="this.debugUp" x="57" y="2" width="50" height="50" rx="16" fill="#00c560" />
          <rect v-if="!this.debugUp" x="57" y="2" width="50" height="50" rx="16" fill="#ff5050" />
          <rect v-if="this.debugDown" x="57" y="57" width="50" height="50" rx="16" fill="#00c560" />
          <rect v-if="!this.debugDown" x="57" y="57" width="50" height="50" rx="16" fill="#ff5050" />
          <rect v-if="this.debugLeft" x="2" y="57" width="50" height="50" rx="16" fill="#00c560" />
          <rect v-if="!this.debugLeft" x="2" y="57" width="50" height="50" rx="16" fill="#ff5050" />
          <rect v-if="this.debugRight" x="112" y="57" width="50" height="50" rx="16" fill="#00c560" />
  ...
  <div :class="{ next_wrapper: !darkMode, next_dark_wrapper: darkMode }">
    ...
    <div class="next_cell" ref="nextRef" :style="`background-color: ${bgCellColor(next.num)}`">
      <div class="next_num" v-if="next.num >= 6"
        :style="`font-size:${nextSize(next.num)}px; color:${color(next.num)};`">
        {{ next.num }}
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    dragOver(e) {
      ...
      if (!this.hasDirection && (Math.abs(this.difY) > 2 || Math.abs(this.difX) > 2)) {
        //如果没确定方向
        this.hasDirection = true //该次拖拽确定方向则不允许更改
        this.delayHandle()
        if (Math.abs(this.difY) > Math.abs(this.difX)) {
          //上下
          if (this.difY > 0) {
            this.direction = 'down'
            this.setMoveDown()
    ...
    drop() {
      ...
      if (
        (this.direction == 'up' && this.difY > -60) ||
        (this.direction == 'down' && this.difY < 60) ||
        (this.direction == 'left' && this.difX > -40) ||
        (this.direction == 'right' && this.difX < 40)
      ) {
        this.moveBack()
        return
      }
      ...
      this.delete()
      if (
        (this.direction == 'up' && this.CanMoveUp()) ||
        (this.direction == 'down' && this.CanMoveDown()) ||
        (this.direction == 'left' && this.CanMoveLeft()) ||
        (this.direction == 'right' && this.CanMoveRight())
      ) {
        setTimeout(() => {
          this.addAndCheck()
        }, 10)
    ...
    moveTo() {
      ...
      for (let i = 0; i < this.cells.length; i++) {
        if (this.cells[i].canMove || this.cells[i].canMerge) {
          moveToAnimate(
            this.$refs.cellsRef[i],
            this.$refs.shadowsRef[i],
            this.cells[i],
            this.direction,
            this.mergeDelayTime,
          )
    ...
    merge() {
      ...
      for (let i = 0; i < this.cells.length; i++) {
        ...
        if (tmp.canMerge) {
          ...
          idFrom = setTimeout(() => {
            mergeFromAnimate(
              this.$refs.cellsRef[this.cells.indexOf(tmp)],
              this.$refs.shadowsRef[this.cells.indexOf(tmp)],
              this.mergeEachTime,
            )
          }, this.mergeDelayTime)
          ...
          this.delayStorey.unshift({
            idFrom: idFrom,
            idTo: idTo,
            dom: this.$refs.cellsRef[this.cells.indexOf(tmp)],
            shadow: this.$refs.shadowsRef[this.cells.indexOf(tmp)],
            cell: tmp,
            toNum: toNum,
          })
        }
      ...
      for (let i = 0; i < 4; i++) {
        ...
        for (let j = 0; j < tmp.length; j++) {
          ...
          if (
            j < tmp.length - 1 &&
            tmp[j].y == tmp[j + 1].y - 1 &&
            (tmp[j].num + tmp[j + 1].num == 3 || (tmp[j].num >= 3 && tmp[j].num == tmp[j + 1].num))
          ) {
        ...
        for (let i = 0; i < tmpCells.length; i++) {
          ...
          setTimeout(
            () => {
              gsap.to(this, {
                duration: 0.2,
                totalScore: Number(this.totalScore + this.score(tmpCells[i].num)),
              })
    ...
    borderBottom(num) {
      return borderBottom(num)
    },
    zIndex(cell) {
      return zIndex(cell, this.direction)
    },
    color(num) {
      return color(num)
    },
    ...
    bgCellColor(num) {
      return bgCellColor(num, this.darkMode, this.triggerScore)
    },
    height(num) {
      return height(num)
    },
    bgCardColor(num) {
      return bgCardColor(num, this.triggerScore)
    },
    score(num) {
      switch (num) {
        case 3:
  ...
  mounted() {
    ...
    this.initialCells()
    // this.next.num = 2;
    // this.cells.push({ x: 0, y: 0, moveX: 0, moveY: 0, canMove: false, canDelete: false, canMerge: false, num: 2, id: this.id++ });
    // this.cells.push({ x: 1, y: 0, moveX: 0, moveY: 0, canMove: false, canDelete: false, canMerge: false, num: 3, id: this.id++ });
    // this.cells.push({ x: 2, y: 0, moveX: 0, moveY: 0, canMove: false, canDelete: false, canMerge: false, num: 6, id: this.id++ });
    // this.cells.push({ x: 3, y: 0, moveX: 0, moveY: 0, canMove: false, canDelete: false, canMerge: false, num: 12, id: this.id++ });
    // this.cells.push({ x: 0, y: 1, moveX: 0, moveY: 0, canMove: false, canDelete: false, canMerge: false, num: 24, id: this.id++ });
    // this.cells.push({ x: 1, y: 1, moveX: 0, moveY: 0, canMove: false, canDelete: false, canMerge: false, num: 48, id: this.id++ });
    // this.cells.push({ x: 2, y: 1, moveX: 0, moveY: 0, canMove: false, canDelete: false, canMerge: false, num: 96, id: this.id++ });
    // this.cells.push({ x: 3, y: 1, moveX: 0, moveY: 0, canMove: false, canDelete: false, canMerge: false, num: 192, id: this.id++ });
    // this.cells.push({ x: 0, y: 2, moveX: 0, moveY: 0, canMove: false, canDelete: false, canMerge: false, num: 384, id: this.id++ });
